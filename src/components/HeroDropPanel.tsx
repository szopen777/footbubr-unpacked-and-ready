...
import { Drop, DropSettings, Product, supabase } from '@/lib/supabase';
...
export default function HeroDropPanel({ drop, countdownTarget, pairCount, dropSettings, featuredProduct }: HeroDropPanelProps) {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      await supabase.from('drop_subscribers').insert([
        { email, drop_settings_id: dropSettings?.id || null },
      ]);
    } catch {
  ...
  // If drop_settings says TBD, show coming soon box
  if (dropSettings?.is_tbd || !countdownTarget) {
    return (
      <div className="bg-[#141414]/80 backdrop-blur-md border border-neutral-800/80 rounded-2xl p-5 sm:p-6 animate-fade-in-up delay-300">
        ...
        {featuredProduct && (
          <div className="flex items-center gap-3 bg-black/40 border border-neutral-800 rounded-xl p-3 mb-4">
            <div className="w-12 h-12 rounded-lg overflow-hidden bg-white/5 border border-neutral-800 flex-shrink-0">
              {featuredProduct.images[0] ? (
                <img src={featuredProduct.images[0]} alt={featuredProduct.name} className="w-full h-full object-cover" />
              ) : (
  ...
  // Valid countdown target — show live countdown
    ...
    return (
      <div className="bg-[#141414]/80 backdrop-blur-md border border-neutral-800/80 rounded-2xl p-5 sm:p-6 animate-fade-in-up delay-300">
        <div className="flex items-center gap-2 mb-4">
          <Zap className="w-4 h-4 text-[#FF6B00]" />
          <span className="text-xs font-black text-[#FF6B00] uppercase tracking-wider">
            {dropSettings?.title || drop?.name || 'Drop za chwilę'}
          </span>
        </div>
        ...
        {dropSettings?.subtitle && (
          <p className="text-sm text-neutral-400 mb-3">{dropSettings.subtitle}</p>
        )}
        {featuredProduct && (
          <div className="flex items-center gap-3 bg-black/40 border border-neutral-800 rounded-xl p-3 mb-3">
            <div className="w-12 h-12 rounded-lg overflow-hidden bg-white/5 border border-neutral-800 flex-shrink-0">
              {featuredProduct.images[0] ? (
                <img src={featuredProduct.images[0]} alt={featuredProduct.name} className="w-full h-full object-cover" />
              ) : (
  ...
  // Countdown expired — fallback to coming soon
  return (
    <div className="bg-[#141414]/80 backdrop-blur-md border border-neutral-800/80 rounded-2xl p-5 sm:p-6 animate-fade-in-up delay-300">
      <div className="flex items-center gap-2 mb-3">
        <Zap className="w-4 h-4 text-[#FF6B00]" />
